...
for(changes of supplyChanges){
    if(changes > 0){
        ...
        console.log(`Removed ${Math.abs(changes)} parts.`)
    }
}
// 8. Write a loop to determine the total number of parts available by
//    adding up all the numbers in the 'supplyChanges' array.
let totalParts = 0;
supplyChanges.forEach(element => totalParts += element);
...
const BOX_CAPACITY = 7;
let parts = 572;
let boxesFilled = 0;
let partsLeftover = parts % BOX_CAPACITY;

while(parts >= BOX_CAPACITY){
parts -= BOX_CAPACITY;
boxesFilled++;
}
console.log(`Boxes filled: ${boxesFilled}`);
console.log(`Parts leftover: ${partsLeftover}`)